...
class HomeController extends Controller {
  ...
  async getArticleList() {
    let sql = `SELECT article.id as id,article.title as title,article.introduce as introduce,FROM_UNIXTIME(article.addTime,'%Y-%m-%d' ) as addTime,article.view_count as view_count ,article.introduce_html as introduce_html ,type.typeName as typeName FROM article LEFT JOIN type ON article.type_id = type.Id WHERE article.isTop = 0  AND article.type_id <> 99 ORDER BY article.id DESC`;
    //console.log(sql)
    ...
    //置顶文章
    let sql2 = `SELECT article.id as id,article.title as title,article.introduce as introduce,FROM_UNIXTIME(article.addTime,'%Y-%m-%d' ) as addTime,article.view_count as view_count ,article.introduce_html as introduce_html ,type.typeName as typeName FROM article LEFT JOIN type ON article.type_id = type.Id WHERE article.isTop = 1 ORDER BY article.id DESC`;
